...
function getLV() {
    $.ajax({
        url: 'dataController/selCon.ashx?operation=viewAccess&tableName=rongyubilipeizhi',
        type: 'GET',
        error: function () {
        ...
        success: function (data) {
            if (data != "") {
                ...
                grid = $('#grid').datagrid({
                    ...
                    url: 'dataController/peizhi02.ashx?action=list&ran=' + Math.random(),  //从远程站点请求数据的 URL
                    ...
                    columns: [[
                                { field: 'betw3190', title: '冗余31-90天扣除比例', width: 100, sortable: true, align: 'center' },
                                { field: 'betw91180', title: '冗余91-180天扣除比例', width: 100, sortable: true, align: 'center' }, //fieldfield 列的字段名,title列的标题文字,sortable 是否允许此列被排序
                                { field: 'over181', title: '冗余180天及以上扣除比例', width: 100, sortable: true, align: 'center' },
...
function add() {
    ...
    dlg_Edit_form.url = 'dataController/peizhi02.ashx?action=save';
}
//编辑
function edit() {
    var rows = grid.datagrid('getSelections');
    ...
    if (num == 0) {
        ...
        dlg_Edit.dialog('open');
        dlg_Edit_form.form('load', 'dataController/peizhi02.ashx?action=get&id=' + rows[0].id);
        dlg_Edit_form.url = 'dataController/peizhi02.ashx?action=save&id=' + rows[0].id;
    }
}
...
function del() {
    ...
    if (arr.length > 0) {
        $.messager.confirm('提示信息', '您确认要删除吗?', function (data) {
            if (data) {
                $.ajax({
                    url: 'dataController/peizhi02.ashx?action=del&id=' + arr2str(arr),
                    type: 'GET',
                    timeout: 1000,
                    error: function () {
...
//搜索全部
function showAll() {
    grid.datagrid({ url: 'dataController/peizhi02.ashx?action=list' });
}
...
function SearchOK() {
    ...
    grid.datagrid({ url: 'dataController/peizhi02.ashx?action=query', queryParams: { title: s_title } });
}
