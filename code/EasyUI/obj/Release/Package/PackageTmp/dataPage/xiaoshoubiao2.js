        ...
        success: function (data) {
            if (data != "") {
                ...
                grid = $('#grid').datagrid({
                    ...
                    url: 'dataController/xiaoshoubiao2.ashx?action=list&ran=' + Math.random(),  //从远程站点请求数据的 URL
                    ...
                    columns: [[
                                { field: 'sale', title: '销售账户', width: 200, sortable: true, align: 'center' }, //fieldfield 列的字段名,title列的标题文字,sortable 是否允许此列被排序
                                { field: 'no', title: '产品编号', width: 150, sortable: true, align: 'center' },
...
function add() {
    ...
    dlg_Edit_form.url = 'dataController/xiaoshoubiao2.ashx?action=save';
}
//编辑
function edit() {
    var rows = grid.datagrid('getSelections');
    ...
    if (num == 0) {
        ...
        dlg_Edit.dialog('open');
        dlg_Edit_form.form('load', 'dataController/xiaoshoubiao2.ashx?action=get&id=' + rows[0].id);
        dlg_Edit_form.url = 'dataController/xiaoshoubiao2.ashx?action=save&id=' + rows[0].id;
    }
}
...
function del() {
    ...
    if (arr.length > 0) {
        $.messager.confirm('提示信息', '您确认要删除吗?', function (data) {
            if (data) {
                $.ajax({
                    url: 'dataController/xiaoshoubiao2.ashx?action=del&id=' + arr2str(arr),
                    type: 'GET',
                    timeout: 1000,
                    error: function () {
...
//搜索全部
function showAll() {
    grid.datagrid({ url: 'dataController/xiaoshoubiao2.ashx?action=list' });
}
...
function SearchOK() {
    ...
    grid.datagrid({ url: 'dataController/xiaoshoubiao2.ashx?action=query', queryParams: { title: s_title} });
}
