...
function getLV() {
    $.ajax({
        url: 'dataController/selCon.ashx?operation=viewAccess&tableName=yonghugaunli',
        type: 'GET',
        error: function () {
        ...
        success: function (data) {
            if (data != "") {
                ...
                grid = $('#grid').datagrid({
                    ...
                    url: 'dataController/user1.ashx?action=list&ran=' + Math.random(),  //从远程站点请求数据的 URL
...
function add() {
    ...
    dlg_Edit_form.url = 'dataController/user1.ashx?action=save';
}
//编辑
function edit() {
    var rows = grid.datagrid('getSelections');
    ...
    if (num == 0) {
        ...
        dlg_Edit.dialog('open');
        dlg_Edit_form.form('load', 'dataController/user1.ashx?action=get&id=' + rows[0].id);
        dlg_Edit_form.url = 'dataController/user1.ashx?action=save&id=' + rows[0].id;
    }
}
...
function del() {
    ...
    if (arr.length > 0) {
        $.messager.confirm('提示信息', '您确认要删除吗?', function (data) {
            if (data) {
                $.ajax({
                    url: 'dataController/user1.ashx?action=del&id=' + arr2str(arr),
                    type: 'GET',
                    timeout: 1000,
                    error: function () {
...
//搜索全部
function showAll() {
    grid.datagrid({ url: 'dataController/user1.ashx?action=list' });
}
...
function SearchOK() {
    ...
    grid.datagrid({ url: 'dataController/user1.ashx?action=query', queryParams: { title: s_title } });
}
