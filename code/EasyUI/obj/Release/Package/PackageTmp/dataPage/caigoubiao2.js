...
function add() {
    ...
    dlg_Edit_form.url = 'dataController/caigoubiao2.ashx?action=save';
}
//编辑
function edit() {
    var rows = grid.datagrid('getSelections');
    ...
    if (num == 0) {
        ...
        dlg_Edit.dialog('open');
        dlg_Edit_form.form('load', 'dataController/caigoubiao2.ashx?action=get&id=' + rows[0].id);
        dlg_Edit_form.url = 'dataController/caigoubiao2.ashx?action=save&id=' + rows[0].id;
    }
}
...
function del() {
    ...
    if (arr.length > 0) {
        $.messager.confirm('提示信息', '您确认要删除吗?', function (data) {
            if (data) {
                $.ajax({
                    url: 'dataController/caigoubiao2.ashx?action=del&id=' + arr2str(arr),
                    type: 'GET',
                    timeout: 1000,
                    error: function () {
...
//搜索全部
function showAll() {
    grid.datagrid({ url: 'dataController/caigoubiao2.ashx?action=list' });
}
...
function SearchOK() {
    ...
    grid.datagrid({ url: 'dataController/caigoubiao2.ashx?action=query', queryParams: { title: s_title } });
}
