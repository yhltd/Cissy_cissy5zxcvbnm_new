...
public class Handler : IHttpHandler 
{
    ...
    private string getSingleData()
    {
        ...
        string sqlexe = @"select id,account,country from Country where id=" + id + "";
    ...
    private string getData(string action)
    {
        ...
        sqlexe = @"select id,account,country from Country " + sWhere + " order by " + sort + " " + order;
        DataTable dt = SqlHelper.dataTable(sqlexe);
    ...
    private string delete()
    {
        string sReturnJson = string.Empty;
        string id = ParamsofEasyUI.RequstString("id");
        string sqlexe = string.Format("delete from Country where id in ({0})", id);
    ...
    private string save()
    {
        ...
        string country = ParamsofEasyUI.RequstForm("country");
        string sqlexe = string.Empty;
        if (id.Length > 0)
        {
            sqlexe = string.Format("update Country set account='{0}',country='{1}' where id={2}",
   account, country, Convert.ToInt32(id));
            if (SqlHelper.ExecuteUpdate(sqlexe))
                sReturnJson = "{success:true}";
            else
                sReturnJson = "{success:false}";
        ...
        {
            sqlexe = string.Format("insert Into Country (account, country) values ('{0}','{1}')",
               account, country);
            if (SqlHelper.ExecuteUpdate(sqlexe))
                sReturnJson = "{success:true}";
            else
