...
public class Handler : IHttpHandler 
{
    ...
    private string getData(string action)
    {
        ...
        else if (action.Equals("query"))
        {
            ...
            string stitlee = ParamsofEasyUI.RequstString("titlee");
            if (!string.IsNullOrEmpty(stitle) && !string.IsNullOrEmpty(stitlee))
                sWhere = "WHERE Sales.sunSKU like '%" + stitle + "%' AND Sales.account = '" + stitlee + "'";
            else if (!string.IsNullOrEmpty(stitle))
                sWhere = "WHERE Sales.sunSKU like '%" + stitle + "%'";
            else if (!string.IsNullOrEmpty(stitlee))
                sWhere = "WHERE Sales.account ='" + stitlee + "'";
        }
...
FROM
    ...
    " + sWhere + "GROUP BY Sales.sunSKU,Sales.account order by " + sort + " " + order;
