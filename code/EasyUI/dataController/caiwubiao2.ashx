...
public class Handler : IHttpHandler 
{
    ...
    private string getSingleData()
    {
        ...
        string sqlexe = @"select * from Trial where id=" + id + "";
    ...
    private string getData(string action)
    {
        ...
        sqlexe = @"select * from Trial" + sWhere + " order by " + sort + " " + order;
        DataTable dt = SqlHelper.dataTable(sqlexe);
    ...
    private string save()
    {
        ...
        string rate = ParamsofEasyUI.RequstForm("rate");
        string name = ParamsofEasyUI.RequstForm("name");
        string commission = ParamsofEasyUI.RequstForm("commission");
        string advertisment = ParamsofEasyUI.RequstForm("advertisment");
        string returnRate = ParamsofEasyUI.RequstForm("returnRate");
        string commissionRate = ParamsofEasyUI.RequstForm("commissionRate");
        string sqlexe = string.Empty;
        if (id.Length > 0)
        {
            sqlexe = string.Format("update Trial set sale='{0}',rate='{1}',name='{2}',commission='{3}',advertisment='{4}',returnRate='{5}',commissionRate='{6}'  where id={7}",
    sale, rate, name, commission, advertisment, returnRate, commissionRate, Convert.ToInt32(id));
            if (SqlHelper.ExecuteUpdate(sqlexe))
                sReturnJson = "{success:true}";
            else
                sReturnJson = "{success:false}";
        ...
        {
            sqlexe = string.Format("insert Into Trial (sale, rate, name, commission, advertisment, returnRate, commissionRate) values ('{0}','{1}','{2}','{3}','{4}','{5}','{6}')",
               sale, rate, name, commission, advertisment, returnRate, commissionRate);
            if (SqlHelper.ExecuteUpdate(sqlexe))
                sReturnJson = "{success:true}";
            else
