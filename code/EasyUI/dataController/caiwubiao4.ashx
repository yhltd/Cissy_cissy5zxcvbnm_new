...
public class Handler : IHttpHandler
{
    ...
    private string save()
    {
        ...
        if (id.Length > 0)
        {
            sqlexe = string.Format("update Sale set account='{0}',name='{1}',sunSKU='{2}',star='{3}' where id={4}",
   account, name, sunSKU, star, Convert.ToInt32(id));
            if (SqlHelper.ExecuteUpdate(sqlexe))
                sReturnJson = "{success:true}";
            else
                sReturnJson = "{success:false}";
        ...
        {
            sqlexe = string.Format("insert Into Sale (account, name, sunSKU, star) values ('{0}','{1}','{2}','{3}')",
               account, name, sunSKU, star);
            if (SqlHelper.ExecuteUpdate(sqlexe))
                sReturnJson = "{success:true}";
            else
