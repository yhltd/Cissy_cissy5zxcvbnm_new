...
public class Handler : IHttpHandler 
{
    ...
    private string getSingleData()
    {
        ...
        string sqlexe = @"select id,account,pwd,LV from Account where id=" + id + "";
    ...
    private string save()
    {
        ...
        string LV = ParamsofEasyUI.RequstForm("LV");
        string sqlexe = string.Empty;
        if (id.Length > 0)
        {
            sqlexe = string.Format("update Account set account='{0}',pwd='{1}',LV='{2}' where id={3}",
   account, pwd, LV, Convert.ToInt32(id));
            if (SqlHelper.ExecuteUpdate(sqlexe))
                sReturnJson = "{success:true}";
            else
                sReturnJson = "{success:false}";
        ...
        {
            sqlexe = string.Format("insert Into Account (account, pwd, LV) values ('{0}','{1}','{2}')",
               account, pwd, LV);
            if (SqlHelper.ExecuteUpdate(sqlexe))
                sReturnJson = "{success:true}";
            else
